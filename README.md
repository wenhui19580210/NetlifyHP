...
- **会社情報管理**: 基本情報・連絡先・カラーテーマを一括編集
  - **ファビコン管理**: ブラウザタブ用・フッター用・ヒーローセクション用の3種類のアイコンを個別に設定可能
- **デザインカスタマイズ**: 
...
### v1.5.1 (2025-10-31) - 現在のバージョン

- 🖼️ **ブラウザタブ用ファビコン機能の追加**
  - `browser_favicon_url` カラムの追加（マイグレーション: 20251031000001）
  - 管理画面でのファビコンアップロード機能（既存）
  - 用途別アイコン管理: ブラウザタブ / フッター / ヒーローセクション
  - 自動フォールバック機能（未設定時はデフォルトアイコンを使用）
- 📚 **ドキュメント整備**
  - ファビコン設定ガイド（FAVICON_SETUP_GUIDE.md）
  - マイグレーション実行ガイドの更新
  - テスト手順書の追加（TESTING_GUIDE.md）
...
- **[FAVICON_SETUP_GUIDE.md](FAVICON_SETUP_GUIDE.md)** - ファビコン設定ガイド
